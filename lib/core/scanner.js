...
function scanDeps(sourcePathOrOps, callbacks)
{
	var ops = {};
	...
	if(callbacks.cache.used[ops.path]) return callbacks.cache.used[ops.path];
	
	if(!callbacks.moduleCount) callbacks.moduleCount = 0;
	callbacks.moduleCount++;
                                      
	
	var depInfo = callbacks.cache.used[ops.path] = { path: ops.path, name: ops.name || ops.path },
	...
	required.forEach(function(dep)
	{
		...
		{
			return callbacks.error({ path: dep, source: ops.path });
		}    
		...
		if(info = scanDeps({ path: dep, name: newDep.name }, callbacks))
		{
			/*if(newDep.pkg)
			{
				// var depDir
	...
	if(pkg)
	{
		...
		var pmops = {
			...
			body: 'module.exports = require("'+ops.name+'")'
		}

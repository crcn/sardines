 ...
 var jsFiles = function(file) {
 	...
 	if(stat.isDirectory()) {
 		fs.readdirSync(file).forEach(function(subFile) {
 			...
 			var subPath = file + '/' + subFile;

 			files = files.concat(jsFiles(subPath));
 		});
...
var combine = function(ops) {
	...
	var getIncludeDetails = function(inc, origin) {
		...
		if(inc.split('/').length == 1 && (newPath = coreModulePath(inc))) {
	...
	var getIncludeDetails2 = function(inc, origin) {
		var details = getIncludeDetails(inc, origin);

		if(details) details.alias = details.alias.replace(/\.coffee$/,'.js');
		return details;
	}
	...
	var required = function(body, ops) {
		...
		var	origin = {
			dir: path.dirname(path.normalize(ops.script)),
		...
		(body.match(/require\(["'].*?["']\)/g) || []).forEach(function(req) {
			...
			inc = getIncludeDetails2(path, origin);
		...
		//additional macro for loading in entire directories
		(body.match(/\/\/#include ([^\n]+)/g) || []).forEach(function(req) {

			var pt = req.match(/\s([^\n]+)/)[1];

			var fullPath = path.normalize(origin.dir + '/' + pt);

			jsFiles(fullPath).forEach(function(file) {
				includes.push(getIncludeDetails2(file.replace(origin.dir, '.'), origin));
			});
		});
	...
	var include = function(ops) {
		...
		if(!ops.alias) ops.alias = realpath;
		...
		if(scriptInfo.path.match(/\.coffee$/)) {
		...
		if(findPackageMainScript(ops.script) == ops.script) {
			...
			console.log('including module: %s', ops.script.bold);
	...
	/*ops.include.forEach(function(src) {
		...
		if(typeof src == 'object') {
			...
			if(src.substr(0,1) == '.') src = process.cwd() + '/' + src;
			
			var stat = fs.statSync(src);

			if(stat.isDirectory()) {
				eachJSFile(fs.realpathSync(src), function(js) {
					include({ script: js });
				});
				return;
			} else {
